            ...
            <div class="col-lg-3 col-md-4">
                <div class="dashboard_menu">
                    <ul class="nav nav-tabs flex-column" role="tablist">
                      ...
                      <li class="nav-item">
                        <a class="nav-link" href="<?php echo site_url('logout'); ?>"><i class="ti-lock"></i>Logout</a>
                                            ...
                                            <?php else: ?>
                                                <?php foreach ($orders as $order): ?>
                                                    <tr>
                                                        <td>
                                                            <?php 
                                                            // Display order_id for customers, suborder_id for merchants
                                                            if ($user['user_type'] === 'customer') {
                                                                echo '#' . esc($order['order_id']);
                                                            } else {
                                                                echo '#' . esc($order['suborder_id']);
                                                            }
                                                            ?>
                                                        </td>
                                                        <td>
                                                            <?php 
                                                            // Use order_date for customers, created_at for suborders (merchants)
                                                            $date = $user['user_type'] === 'customer' ? $order['order_date'] : $order['created_at'];
                                                            echo date('F j, Y', strtotime($date));
                                                            ?>
                                                        </td>
                                                        <td><?php echo ucfirst(esc($order['status'])); ?></td>
                                                        <td>
                                                            <?php 
                                                            // Use total_amount for customers, subtotal for suborders (merchants)
